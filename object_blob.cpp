...
void object_blob::add_data(char *name, void *data_in, int type, int length) {
  ...
  data[num_data - 1] =
      (char *)malloc(sizeof(char) * data_size[num_data - 1] * length);
...
*/
void object_blob::massage_data(int /* type */, void *data_in, int length,
                               int size) {
...
char *object_blob::save_blob(int *size) {
  ...
  for (int d = 0; d < num_data; d++) {
    blob_size += sizeof(int) * 4 + strlen(data_name[d]) + 1 +
                 data_length[d] * data_size[d];
